...
export const DashboardView = () => {
  const { clients, user, viewClientDetails, setActiveTab, setIsSearchOpen, activeProgram } = useCRM();
  ...
  return (
    <main className="pt-20 md:pt-28 px-4 sm:px-6 max-w-5xl mx-auto w-full min-w-0 flex flex-col gap-md sm:gap-lg pb-28 sm:pb-32 animate-in fade-in duration-200">
      {/* Welcome Banner */}
      <div className="bg-gradient-to-r from-primary/95 to-secondary p-5 sm:p-lg rounded-2xl text-on-primary shadow-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 min-w-0">
        <div className="flex items-center gap-4 sm:gap-5 min-w-0">
          <img 
            src={userPhoto} 
            alt={user.name}
            referrerPolicy="no-referrer"
            className="w-14 h-14 sm:w-20 sm:h-20 rounded-full object-cover border-4 border-white/30 shadow-md shrink-0 ring-2 ring-white/20 bg-primary-container"
            onError={(e) => {
          ...
          <div className="min-w-0">
            <div className="flex items-center flex-wrap gap-1.5 text-primary-fixed text-xs uppercase tracking-wider font-bold mb-1">
              <span className="material-symbols-outlined text-sm text-emerald-400">verified_user</span> 
              <span>Google Account Connected</span>
              <span className="px-2 py-0.5 rounded-full bg-white/20 text-white text-[10px] font-bold normal-case tracking-normal flex items-center gap-1">
                <span className="material-symbols-outlined text-[11px]">layers</span>
                {activeProgram}
              </span>
            </div>
            <h1 className="font-headline-lg text-xl sm:text-2xl font-bold text-white leading-snug truncate">
              Welcome back, {user.name}!
            </h1>
            <p className="font-body-md text-xs sm:text-sm text-primary-fixed/90 mt-1 max-w-xl">
        ...
        <button
          ...
          className="bg-surface text-primary font-bold px-5 py-2.5 rounded-xl shadow hover:bg-surface-container transition-colors flex items-center gap-2 shrink-0 self-end sm:self-center whitespace-nowrap"
        >
      ...
      <PromoBanner className="mt-2 sm:mt-4" />
    </main>
  );
};
